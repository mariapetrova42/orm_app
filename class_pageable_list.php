...
class pg_list {
   ...
   function get_page_url($page){
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    function get_top_item($sort_by, $specific_column = null) {
        // Check if the sort_by is valid and sortable
        $validSortBy = false;
        foreach ($this->columns as $col) {
            if ($col->column_name === $sort_by && $col->sortable) {
                $validSortBy = true;
                break;
            }
        }
        ...
        if (!$validSortBy) {
            die("Invalid or non-sortable column specified for get_top_item.");
        }

        // Clone the object to avoid modifying the original state
        $tempList = clone $this;

        // Set the sorting criterion for the temporary list
        $tempList->sort_by = $sort_by;
        $tempList->sort_dir = 'DESC'; // Assuming descending order

        // Initialize the temporary list
        $tempList->init_list();

        // Get the top item
        $result = lib::db_query($tempList->query . " LIMIT 1");
        $row = $result->fetch_assoc();
        mysqli_free_result($result);

        // Check if a specific column is requested
        if ($specific_column !== null && array_key_exists($specific_column, $row)) {
            return $row[$specific_column];
        }
